...
canvas.addEventListener('click', function(event) {  
  ...
  //console.log(prevR)
    ...
    if(prevx===100000000000||prevR==R.toFixed(2)||prevY==Y.toFixed(2)){
  c.beginPath();
  c.arc(locx, locy, 1, 0, 2 * Math.PI);
  c.fillStyle = 'red';
  c.fill();
  ...
  //moving along constant X circle primary
    ...
    if(delG>0){
        ...
        objects.push(new CircuitElement(cap.toFixed(2),'images/shuntind.png'))
        drawCircuit()
        ...
        objects.push(new CircuitElement(ind.toFixed(2),'images/shuntcap.png'))
        drawCircuit()
